...
import { User, LogOut, Settings, Moon, Sun, PlusCircle, Search, X } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ onCreateNote, searchQuery, onSearchChange, isSidebarOpen, onCloseSidebar }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Content */}
      <div className={`fixed inset-y-0 left-0 w-80 bg-white/5 dark:bg-gray-900/5 backdrop-blur-lg border-r border-white/10 flex flex-col h-full z-50 transform transition-transform duration-300 md:relative md:transform-none ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-8">
            ...
            <button onClick={onCloseSidebar} className="md:hidden p-1 text-gray-500 dark:text-gray-400">
              <X className="w-6 h-6" />
            </button>
          </div>
